...
public class RetirementReceiptController {
    ...
    public void registerRetirementReceipt(
        ...
        if(!resultMap.get("errorCode").equals("0")) {
            ...
            var.add("errorCode", resultMap.get("errorCode"));
            var.add("errorMsg", resultMap.get("errorMsg"));
        }
    }

    @PostMapping("modifyRetirementApply")
    public void modifyRetirementApply(@RequestAttribute("reqData") PlatformData reqData,
                                      @RequestAttribute("resData") PlatformData resData) {
        ResultTO resultTO = null;
        String empCode = reqData.getVariable("empCode").getString();
        String retirementDate = reqData.getVariable("retirementDate").getString();
        RetirementPersonTO retirementPersonTO = new RetirementPersonTO();
        retirementPersonTO.setEmpCode(empCode);
        retirementPersonTO.setRetirementDate(retirementDate);
        resultTO = retirementMgmtService.modifyRetirementApply(retirementPersonTO);

        if(!resultTO.getErrorCode().equals("0")) {
            VariableList var = resData.getVariableList();
            var.add("errorCode", resultTO.getErrorCode());
            var.add("errorMsg", resultTO.getErrorMsg());
        }

    }

    @PostMapping("findRetirementReceiptlist")
    public void findRetirementList(@RequestAttribute("reqData") PlatformData reqData,
        ...
        datasetBeanMapper.beansToDataset(resData, retirementPersonList, RetirementPersonTO.class);
...
//        map.put("errorCode", 0);
    ...
    @PostMapping("report")
    public void retirementReceiptReport(HttpServletRequest request,
//                                        HttpServletResponse response,
                                        @RequestAttribute("reqData") PlatformData reqData,
                                        @RequestAttribute("resData") PlatformData resData) throws JRException, IOException { // 재직증명서 신청

//        String empCode = request.getParameter("empCode"); // 해쉬맵 생성, URL에서 보낸 파라미터값을 맵에 담는다
        String empCode = reqData.getVariable("empCode").getString(); // 해쉬맵 생성, URL에서 보낸 파라미터값을 맵에 담는다
...
//        response.setContentType("application/pdf"); // PDF형식으로 변환!
        ...
        // // 본인 이클립스 경로
         JasperExportManager.exportReportToPdfFile(jasperPrint,
            (request.getServletContext().getRealPath("/report/retirementReceipt_"+to.getEmpName()+"_"+to.getToday()+".pdf")));
